...
class account_account(models.Model):
    ...
    @api.multi
    @api.depends('child_id')
    def _compute_liability_ids(self): 
        for line in self:
            for lines in line.child_id:
                print 'child_id-============================',lines
...
#             print 'test==============='
    ...
    move_lines = fields.One2many('account.move.line', 'account_id', 'Journal Items', domain=['|',('cloud', '=', 'no'),('cloud', '=', None)])
    move_lines2 = fields.One2many('account.move.line', 'account_id', 'Journal Items', domain=['|',('cloud', '=', 'yes'),('cloud', '=', None)])
...
#                 print 'accounts=================', line.accounts_names
    ...
    cloud = fields.Selection([
            ('no', 'No'),
            ('yes', 'yes'),
        ], 'Clound', copy=False)
    origin_id = fields.Many2one('account.move', 'Origin Move')
    saved = fields.Boolean('Is Saved', default=False)    
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        vals['saved'] = True
        if vals.get('origin_id'):
            journal_entry = self.pool.get('account.move').browse(cr, uid, vals.get('origin_id'))
            print 'jouwssdf==================', journal_entry
            journal_entry.cloud = 'no'
        if vals.get('line_id'):
    ...
    def open_cloud_entry(self):
        for rec in self:
            ...
            return {
                ...
                'view_id':'view_move_form_hiworth',
                'type':'ir.actions.act_window',
                'target': 'new',
                # 'res_id':res_id,
                'context':{'default_cloud':'yes','default_journal_id':rec.journal_id.id, 'default_date':rec.date, 'default_perios_id': rec.period_id.id, 'origin_id': rec.id, 'default_origin_id': rec.id},
            }

    def default_get(self, cr, uid, fields, context=None):
        res = super(account_move, self).default_get(cr, uid, fields, context=context)
        lines = []
        if context.get('origin_id'):
            for lines2 in self.pool.get('account.move').browse(cr, uid, context.get('origin_id')).line_id:

                line = {
                    'account_id': lines2.account_id.id,
                    'cheque_no': lines2.cheque_no,
                    'invoice_no_id2': lines2.invoice_no_id2.id,
                    'cloud': 'yes',
                    'name': lines2.name,
                    'debit': lines2.debit,
                    'credit': lines2.credit,
                }
                lines.append(line)
                
        print 'origin==================',lines
        res.update(line_id=lines)
        return res
...
#                 line.is_posted = True
    ...
    acc_balance = fields.Float(related='account_id.balance', string='Available Balance')
    ...
    is_posted = fields.Boolean(string="Is Posted", default=False, copy=False)
    # attachment_ids = fields.One2many('ir.attachment', 'move_line_id', 'Attachments')
    cloud = fields.Selection(related='move_id.cloud', store=True, string='Clound', copy=False)
